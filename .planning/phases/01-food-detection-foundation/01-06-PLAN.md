...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/mobile/src/ml/InferenceRouter.ts"
      ...
      via: "SQLite query for ingredient lookup after dish classification"
      pattern: "knowledge.*query|sqlite.*dish"
    - from: "apps/mobile/src/ml/InferenceRouter.ts"
...
2. **Create `apps/mobile/src/ml/InferenceRouter.ts`:**
   ...
   export class InferenceRouter {
     ...
     private async lookupIngredients(dishName: string): Promise<Ingredient[]> {
       // Query the bundled SQLite knowledge graph
       // Use the recursive CTE from knowledge-graph/query.py, ported to SQL string
       // If no exact match, use FTS5 best-guess search (per locked decision: always return something)
     }
   }
   ```
